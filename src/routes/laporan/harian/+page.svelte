...
{:else}
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-[90%] mx-auto p-4 rounded-[16px] bg-white shadow-md mt-2.5">
  {#each $filteredReports as report}
    <a href={`/laporan/${report.SR_ID}`} class="flex items-center gap-4 bg-[#f4f6ff] p-4 rounded-xl hover:shadow transition-shadow duration-200">
      <!-- Icon -->
      <div class="flex-shrink-0 bg-[#cad4fd] rounded-md p-3 flex items-center justify-center">
        <svg 
      ...
      <!-- Text -->
      <div>
        <p class="font-semibold text-[#12343b]">{report.SR_TITLE}</p>
